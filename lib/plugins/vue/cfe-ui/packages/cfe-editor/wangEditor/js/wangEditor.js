...
// editor api
    _e(function (E, $) {
        ...
        E.fn.updateValue = function () {
            ...
            $valueContainer.selectionStart = value.length;
        };
            ...
            if (console == null) {
                ...
                E[value] = function (info) {
                    ...
                    if (E.config && E.config.printLog) {
                        console[value]('cfe-Editor提示: ' + info);
                    }
                };
            }
        });
...
// 全局配置
    _e(function (E, $) {
        ...
        // 自定义上传时传递的参数（如 token）
        E.config.uploadImgParams = {
            /* token: 'abcdef12345' */
        };

        //上传文件
        E.config.uploadFileFns = {};
        E.config.uploadFileParams = {
            /* token: 'abcdef12345' */
        };
        // 自定义上传是的header参数
...
// 全局UI
    _e(function (E, $) {
        ...
        E.UI.menus = {
            ...
            file: {
                normal: '<a href="#" tabindex="-1"><i class="wangeditor-menu-img-upload"></i></a>',
                selected: '.selected'
            },
            video: {
...
// file 文件菜单 by zhou
    _e(function (E, $) {
        ...
        E.createMenu(function (check) {
            ...
            // tab
            var $uploadTab = $('<a href="#">上传文件</a>');
            $tabContainer.append($uploadTab)
            // var $linkTab = $('<a href="#">网络文件</a>');
            // $tabContainer.append($uploadTab).append($linkTab);
            ...
            editor.ready(function () {
                ...
                var uploadImgUrl = config.uploadImgUrl;
                var customUpload = config.customUpload;
                var linkImg = config.hideLinkFile;
                var $uploadImgPanel;

                if (uploadImgUrl || customUpload) {
                    // 第一，暴露出 $uploadContent 以便用户自定义 ！！！重要
                    editor.$uploadFileContent = $uploadContent;
...
// 上传图片事件
    _e(function (E, $) {
        ...
        E.plugin(function () {
            ...
            var downloadImgBaseUrl = editor.config.downloadImgBaseUrl;
            // -------- 定义load函数 --------
            fns.onload || (fns.onload = function (resultText, xhr) {
                ...
                if (resultText.errorCode == 0) {
                    ...
                    var html = '<img src="' + downloadImgBaseUrl + src + '" alt="' + originalName + '" style="max-width:100%;"/>';
...
// xhr 上传图片
    _e(function (E, $) {
        ...
        E.plugin(function () {
            ...
            // 获取配置中的上传事件
            var uploadFileFns = config.uploadFileFns;
            var fileOnload = uploadFileFns.onload;
            var fileOntimeout = uploadFileFns.ontimeout;
            var fileOnerror = uploadFileFns.onerror;
            ...
            // -------- xhr 上传文件  by zhou --------
            editor.xhrUploadFile = function (opt) {
                // opt 数据
                var event = opt.event;
                var fileName = opt.filename || '';
                var base64 = opt.base64;
                var fileType = opt.fileType || 'application/pdf'; // 无扩展名则默认使用 pdf
                var name = opt.name || 'wangEditor_upload_file';
                var loadfn = opt.loadfn || fileOnload;
                var errorfn = opt.errorfn || fileOnerror;
                var timeoutfn = opt.timeoutfn || fileOntimeout;

                // 上传参数（如 token）
                var params = editor.config.uploadFileParams || {};

                // headers
                var headers = editor.config.uploadHeaders || {};

                // 获取文件扩展名
                var fileExt = 'pdf';  // 默认为 png
            ...
            editor.xhrUploadImg = function (opt) {
                ...
                // 上传参数（如 token）
                var params = editor.config.uploadImgParams || {};
...
// upload img 插件
    ...
    _e(function (E, $) {
        ...
        E.plugin(function () {
            ...
            // 获取editor的上传dom
            var $uploadFileContent = editor.$uploadFileContent;
            ...
            // 自定义UI，并添加到上传dom节点上
            var $uploadIcon = $('<div class="upload-icon-container"><i class="wangeditor-menu-img-upload"></i></div>');
            $uploadFileContent.append($uploadIcon);
...
// h5 方式上传图片
    _e(function (E, $) {
        ...
        UploadFile.fn.upload = function (file) {
            ...
            // var uploadFileName = editor.config.uploadFileFileName || 'wangEditorH5File';
            var fileOnload = uploadFileFns.onload;
            var fileOntimeout = uploadFileFns.ontimeout;
            var fileOnerror = uploadFileFns.onerror;
                ...
                if (self.type == 'file') {
                    editor.xhrUploadFile({
                        ...
                        loadfn: function (resultText, xhr) {
                            ...
                            // 执行配置中的方法
                            var editor = this;
                            fileOnload.call(editor, resultText, xhr);
                        },
                        errorfn: function (xhr) {
                            clearInput();
                            if (E.isOnWebsite) {
                                alert('cfeEditor官网暂时没有服务端，因此报错。实际项目中不会发生');
                            }
                            // 执行配置中的方法
                            var editor = this;
                            fileOnerror.call(editor, xhr);
                        },
                        timeoutfn: function (xhr) {
                            clearInput();
                            if (E.isOnWebsite) {
                                alert('cfeEditor官网暂时没有服务端，因此超时。实际项目中不会发生');
                            }
                            // 执行配置中的方法
                            var editor = this;
                            fileOntimeout(editor, xhr);
                        }
                    });
                } else {
                    editor.xhrUploadImg({
...
// form方式上传图片
    _e(function (E, $) {
        ...
        UploadFile.fn.render = function () {
            ...
            // 增加用户配置的参数，如 token
            $.each(editor.config.uploadImgParams, function (key, value) {
