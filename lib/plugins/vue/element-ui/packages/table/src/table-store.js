define(function (require, exports, module) {
  ...
  const getColumnById = require('./util').getColumnById;
  const getRowIdentity  = require('./util').getRowIdentity;


  const sortData = function(data, states)
  {
  ...
  TableStore.prototype.mutations = {
    setData: function(states, data) {
      ...
      var that = this;
      Vue.nextTick(function(){that.table.updateScrollY()}
    )
      ;
    },
  ...
  const doFlattenColumns = function(columns)
  {
    const result = [];
  ...
  TableStore.prototype.updateColumns = function () {
    ...
    states.fixedColumns = _columns.filter(function(column) { return column.fixed === true || column.fixed === 'left'}
    )
    ;
    states.rightFixedColumns = _columns.filter(function(column){ return column.fixed === 'right'}
    )
    ;
    ...
    states.originColumns = [].concat(states.fixedColumns).concat(_columns.filter(function(column) {return !column.fixed} )
    ).
